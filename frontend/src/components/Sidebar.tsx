...
export function Sidebar({ sessions, currentSessionId, onNewSession, onSelectSession, onDeleteSession }: Props) {
  ...
  return (
    <div className="w-48 flex-shrink-0 flex flex-col h-full" style={{ background: "#F7F7F7", borderRight: "1px solid rgba(0,0,0,0.07)" }}>

      {/* Logo */}
      <div className="flex items-center gap-2.5 px-4 py-4" style={{ borderBottom: "1px solid rgba(0,0,0,0.07)" }}>
        <div className="w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: PRIMARY }}>
          <svg className="w-3.5 h-3.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
            <circle cx="12" cy="12" r="9" strokeWidth={2} />
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l2.5 2" />
          </svg>
        </div>
        <span className="text-sm font-semibold tracking-tight" style={{ color: "#1C1C1C" }}>Grove</span>
      </div>
      ...
      <div className="px-3 pt-3 pb-2">
        <button
          ...
          style={{ background: `rgba(28,28,28,0.07)`, color: PRIMARY, border: `1px dashed rgba(28,28,28,0.3)` }}
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <span style={{
                          ...
                          background: isActive ? PRIMARY : "#CFCFCF",
                          transition: "background 0.2s",
                        }} />
                        <span className="truncate text-xs leading-relaxed" style={{ color: isActive ? "#1C1C1C" : "#5F5F5F", fontWeight: isActive ? 500 : 400 }}>
                          {s.title}
                        </span>
                      </div>
                      <div className="text-xs mt-0.5 pl-[13px]" style={{ color: "#B0B0B0" }}>
      ...
      <div className="px-4 py-3" style={{ borderTop: "1px solid rgba(0,0,0,0.07)" }}>
        <div className="text-xs" style={{ color: "#9A9A9A" }}>Research Assistant</div>
      </div>
    </div>
  );
}
