...
- 锚点观察（anchor_observation）：用户自己表达的对这个领域/问题的既有认知或观察（用户说的，或用户明确认同的）；AI 自己提出的分析不算
- 核心假设（key_assumption）：用户持有的假设或判断（用户自己说的，或用户明确认同的）；AI 提出、用户未明确回应的判断不算
...
**归因原则（reply 文本同样适用）**：
只有用户在对话中主动说的内容，才能用"你提到……"表达。
AI 自己在之前轮次提出的分析、维度、候选方向，即使用户表示认同，也不能说"你提到"。
正确示例："我们讨论到的三个维度……" 或 "我之前提出的……你也认可的……"
错误示例："你提到的竞争维度——技术壁垒、场景卡位……"（如果这是 AI 先提出的）
